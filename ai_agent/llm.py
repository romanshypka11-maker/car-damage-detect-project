...
async def generate_car_verdict(
    ...
    car_dict = car_data or {}
    cv_text = ", ".join(cv_reports) if cv_reports else "Архівних фото з аукціонів для цього VIN не знайдено."
    ...
    description = (car_dict.get("description") or "")[:300]
    ...
    request_url = f"{settings.ollama_url.rstrip('/')}/api/generate"
...
async def generate_sql_via_qwen(user_text: str) -> str:
    ...
    try:
        ...
        logger.exception("Error in generate_sql_via_qwen")
        return ""
